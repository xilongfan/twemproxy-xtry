...
/* XTRY
add a server to server pool sp by providing a server configuraton cs
*/
rstatus_t add_server(struct array *server, struct conf_server *cs, struct server_pool *sp)
{
    rstatus_t status;
    struct server *new_server = NULL;

    status = conf_server_each_transform(cs, server);
    if (status != NC_OK) {
        server_deinit(server);
        return status;
    }
    new_server = array_top(server);
    new_server->owner = sp; /* set server owner */

    log_warn("XTRY: ==================================================================> add a new server '%.*s' in pool %"PRIu32" '%.*s'",
              new_server->pname.len, new_server->pname.data, sp->idx, sp->name.len, sp->name.data);


    status = server_each_preconnect(new_server, NULL);
    if (status != NC_OK) {
        log_warn("XTRY: add_server.server_each_preconnect connect to server '%.*s' failed, ignored: %s",
                 new_server->pname.len, new_server->pname.data, strerror(errno));
    }
