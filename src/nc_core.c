...
*/
void
test_add_server(struct instance *nci, struct context *ctx)
{
    rstatus_t status;
    struct conf_server field;
    conf_server_init2(&field);
    status = string_copy(&field.pname, (uint8_t*)"127.0.0.1:6379:1", 16);

    field.weight = 1;
    field.port = 6379;
    status = string_copy(&field.name, (uint8_t*)"127.0.0.1:6379", 14);

    status = string_copy(&field.addrstr, (uint8_t*)"127.0.0.1", 9);
...
/*
    ...
    status = stats_pool_map(&st->current, &ctx->pool);
    status = stats_pool_map(&st->shadow, &ctx->pool);
    status = stats_pool_map(&st->sum, &ctx->pool);
*/
}
